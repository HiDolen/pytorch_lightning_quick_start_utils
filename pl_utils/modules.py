...
class BaseModule(L.LightningModule):
    ...
    def on_train_start(self) -> None:
        ...
        # 记录 lr_schedule
        try:
            scheduler_func = self.training_config.scheduler
            ...
            for step in sorted(steps_to_log):
                if 0 <= step < len(lr_schedule):
                    self.logger.experiment.add_scalar('lr/lr', lr_schedule[step], step)
        except Exception as e:
            print("Record lr_schedule failed. But it's ok.")

    def configure_optimizers(self):
        class JointOptimizer(torch.optim.Optimizer):
            """将多个优化器打包为一个，对外表现为单个优化器。"""

            def __init__(self, optimizers: list):
                param_groups = [dict(group) for opt in optimizers for group in opt.param_groups]
                super().__init__(param_groups, {})
                self.optimizers = optimizers
                # 保证 LambdaLR 修改的是同一份对象
